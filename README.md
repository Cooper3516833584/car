...
每个完整圆周仍先由 ICP 推进连续位姿。小于 `2 cm / 1°` 的单圈变化会作为新鲜有效
观测抑制抖动，但不会前移 ICP 关键帧；低速真实运动会继续相对同一关键帧累积，越过
门限后一次性写入，因此不会在接近目标时冻结里程计。到达配置周期后，墙线定位器使用
ICP 预测把点云转换到墙体坐标系，只使用经过原车日志验证的后墙和右墙作为绝对锚点。
D500 是 360° 雷达，车头转向不会改变这两条固定墙在墙体坐标系中的定义。
直线必须满足最少点数、长度、RMS、轴向角以及两墙方向一致性门限。通过质量检查后，
墙距/航向还必须通过相对 ICP 预测的最大位置和航向残差门限，才按有限增益回写
`RadarOdometry.pose`。纠正后的同一位姿用于全局点云建图和 Navigation，避免定位与
地图使用两套姿态。前墙和左墙仍用于启动矩形边界与碰撞地图，但不再参与在线位姿
回写，避免平行障碍或对向墙误关联。

输入完整圆周还会先按无人机仓库定位逻辑使用的保守范围过滤：距离 `10～650 cm`、
置信度至少 `30`。若墙被遮挡、线段过短、残差过大或只看到其中一面墙，组件不会中断 ICP：有效的单墙
可以只纠正对应坐标和航向；没有有效墙线时继续使用 ICP。该方法用于纠正缓慢累计
漂移，不负责从严重错误的初始位置重新定位；默认候选关联范围为 `45 cm`。默认还将
单次实际墙线修正限制在 `2 cm / 0.5°` 内，超过即拒绝，禁止一次错误墙观测造成位姿
跳变。
...
navigation.set_goal(
    NavigationGoal(
        ...
        position_tolerance_cm=5,
        heading_tolerance_deg=3,
    )
)
navigation.start_navigation()
```
...
`COMPLETED`。正式默认容差为位置 `5 cm`、航向 `3°`，且必须由至少 3 个不同雷达
圆周持续 `0.5 s` 确认；单次 ICP/墙线跳变进入容差不会误报到达。重复的
`(session, seq)` 命令不会重复发车。已有 `STOP_MISSION = 5`
用于取消当前导航。主程序依次返回 V2 `RECEIVED`、`ACCEPTED/REJECTED`，任务结束后
返回 `COMPLETED/FAILED`。

为使无人机和小车直接使用同一组目标坐标，发送端可在小车建图完成、任务开始前发送
`SET_COORDINATE_FRAME = 0x21`。其 payload 为：

```text
command_id:u8 = 0x21
startup_origin_x_cm:i32 LE
startup_origin_y_cm:i32 LE
startup_x_heading_centideg:u16 LE   0..35999，俯视逆时针为正
```

这三个量定义“小车本次启动坐标系 -> 无人机共享坐标系”的固定 SE(2)：前两个量是
小车启动后轴中心在无人机坐标中的位置，最后一个量是“小车启动 +X/车头”相对无人机
`+X` 的逆时针方向。车端接受后会把当前雷达位姿、累计点图、矩形边界、墙线参考和
Navigation 地图一起原子转换；不能在任务执行中切换。随后 `NAVIGATE_TO` 的
`x/y/heading` 可直接填写无人机坐标。每次 main 启动只接受一次坐标同步；若标定值
需要修改，应停车并重启重新建图。重复 `(session, seq)` 仍保持幂等。

若不发送 `0x21`，为兼容 SSH 和旧发送端，坐标仍属于本次启动建立的
`(0,0,0°)` 车头坐标系。车端不会根据无人机当前位姿猜测变换；`0x21` 的数值必须来自
同一物理参考的实测/标定，不能把无人机自身当前坐标误当成小车启动原点。

发送端可以先用
`pack_coordinate_frame_command(CoordinateFrameTransform(...))` 同步坐标系，再用
`pack_navigation_command(NavigationGoal(...))` 生成完整内部
`AA 22` 帧。当前 `ground_station` 原代码尚无 `0x20` 坐标命令，地面站/无人机发送端
必须按上述 payload 增加 `0x20/0x21` 后才能下发共享坐标；车端接收和 ACK 已完成。
